...
//	$data = json_decode(trim(substr(file_get_contents($filename),15)));	
	...
	foreach ($data->tenor_group as $v1){
		...
		$tcontent .=	"<table class=\"table table-condensed\"><thead><tr><th colspan =\"5\" style=\"text-align:center;\">";
		$tcontent .= $currencyname;
		$tcontent .= $preservable.iconv('GB2312','UTF-8', "理财产品收益对比表");
		$tcontent .= "</th></tr><tr><th style=\"text-align:center;\">".iconv('GB2312','UTF-8', "期 限");
		$tcontent .= "</th><th style=\"text-align:center;\">".iconv('GB2312','UTF-8', "银行名称");
		$tcontent .= "</th><th style=\"text-align:center;\">".iconv('GB2312','UTF-8', "近期最高收益");
		$tcontent .= "<sup><font color=\"red\">1</font></sup></th><th style=\"text-align:center;\">".iconv('GB2312','UTF-8', "行业平均");
		...
		foreach ($v1->list as $v2){
			...
			$tcontent .= "<td style=\"text-align:center;width:120px;\">".$v2->issuer_name."</td>";
